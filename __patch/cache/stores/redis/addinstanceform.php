...
class cachestore_redis_addinstance_form extends cachestore_addinstance_form {
    ...
    protected function configuration_definition() {
        ...
        $form->addElement('text', 'dbid', get_string('dbid', 'cachestore_redis'), array('size' => 4));
        $form->setType('dbid', PARAM_TEXT); // We set to text but we have a rule to limit to alphanumext.
        $form->addHelpButton('dbid', 'dbid', 'cachestore_redis');
        $form->addRule('dbid', get_string('dbidinvalid', 'cachestore_redis'), 'regex', '#^[0-9]+$#');
        // CHANGE-.
